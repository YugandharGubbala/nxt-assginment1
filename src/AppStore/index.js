...
class AppStore extends Component {
  state = {seachInput: '', tabInput: tabsList[0].tabId}
  ...
  searchvalue = () => {
    ...
    const result = appsList.filter(each =>
      each.appName.toUpperCase().includes(seachInput.toUpperCase()),
    )
    return result
  }

  getTab = result => {
    const {tabInput} = this.state
    const method = result.filter(each => each.category === tabInput)
    return method
  }
  render() {
    const result = this.searchvalue()
    const filteritems = this.getTab(result)

    return (
      <div className="container">
        <div className='imgcontainer'>
          <h1>APP</h1>
        <img className='appimage' src="https://i.ibb.co/JjYX3Y6/app-store.png" alt="app-store"/>
        <h1>BAZZAR</h1>
        </div>
